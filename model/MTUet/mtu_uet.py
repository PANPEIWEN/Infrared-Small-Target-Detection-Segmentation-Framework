...
from model.MTUUet.vit import ViT
...
class MTUNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=3, block=Res_block, num_blocks=(2, 2, 2, 2),
        ...
        self.conv3_1_1 = self._make_layer(block,
                                          nb_filter[2] + nb_filter[2] + nb_filter[2] + nb_filter[2] + nb_filter[4],
                                          nb_filter[4])
    ...
    def forward(self, input):
        ...
        out = torch.cat([rearrange(self.vit2(x3_0), "b (x y) c -> b c x y", x=32, y=32),
                         rearrange(self.vit3(x2_0), "b (x y) c -> b c x y", x=32, y=32),
                         rearrange(self.vit4(x1_0), "b (x y) c -> b c x y", x=32, y=32),
                         rearrange(self.vit5(x0_0), "b (x y) c -> b c x y", x=32, y=32), out], 1)
