...
class Segformer(nn.Module):
    def __init__(
        ...
        assert all([*map(lambda t: len(t) == 4, (dims, heads, ff_expansion, reduction_ratio,
                                                 num_layers))]), 'only four stages are allowed, all keyword arguments' \
                                                                 ' must be either a single value or a tuple of 4 values'
    ...
    def forward(self, x):
        ...
        return self.up(self.to_segmentation(fused))


if __name__ == '__main__':
    x = torch.rand(8, 3, 512, 512)
    model = Segformer(dims=(64, 128, 320, 512), num_layers=(3, 3, 6, 3))
    out = model(x)
    print(out.size())
